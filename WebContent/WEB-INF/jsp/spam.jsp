	...
	<link href="${pageContext.request.contextPath}/css/style.css" rel="stylesheet" type="text/css" />
		...
		<script>
			function deleteone(mid)
			{
				var a=confirm("确定删除吗");
				if(a){
					location.href="${pageContext.request.contextPath}/user/mailDelete.action?mid="+mid;
				}else{
					
				}
			}
			function backeone(mid)
			{
				var a=confirm("确定还原吗");
				if(a){
					location.href="${pageContext.request.contextPath}/user/mailBack.action?mid="+mid;
				}else{
				...
				<div class="action">
						...
						<div class="pages">
							<table width="90%" border="0" cellspacing="0" cellpadding="0">
								...
								<c:forEach items="${listMails2}" var="mail">
								<tr>
									<td>${mail.title}</td>
									<td>${mail.content}</td>
									<td>${mail.isread}:2表示文件被删除</td>
					    			<td>${mail.time}</td>
					    			<td><a href="#" onclick="deleteone(${mail.mid})">删除</a></td>
					    			<td><a href="#" onclick="backeone(${mail.mid})">还原</a></td>
								</tr>
								</c:forEach>
								</table>
						</div>
